...
cron.schedule("0 0 * * *", async () => {
  ...
  try {
    ...
    for (const member of members) {
      ...
      if (!member.membership?.expiryDate) continue;
      ...
      if (daysLeft === 7 && !member.reminders.day7Sent) {
        await sendMail(
          ...
          emailTemplate(member.fullName, member.membership.expiryDate, 7)
        );
      ...
      if (daysLeft === 3 && !member.reminders.day3Sent) {
        await sendMail(
          member.email,
          "⚠️ Membership Expiring in 3 Days",
          emailTemplate(member.fullName, member.membership.expiryDate, 3)
        );
      ...
      if (daysLeft === 1 && !member.reminders.day1Sent) {
        await sendMail(
          ...
          emailTemplate(member.fullName, member.membership.expiryDate, 1)
        );
...
const emailTemplate = (name: string, expiryDate: Date, days: number) => `
<p>Hello <b>${name}</b>,</p>
